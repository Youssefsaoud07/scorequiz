...
export default function Auth() {
  ...
  const handelLogout = async () => {
    try {
      await firebase.auth().signOut().then((result) => { history.push('/login') })
          ...
          {user ? (
            <div className={classes.sectionDesktop}>
              ...
              >
                ...
                <Typography fontFamily='roboto' style={{ fontSize: 14, paddingLeft: 10 }}> {user.displayName}</Typography>
              </IconButton>
            </div>) : (<div></div>)}
          {user ? (
